...
{
#include <stdint.h> 
  const unsigned wA = %(MATRIX_SIZE)s;
  const unsigned wB = %(MATRIX_SIZE)s;  
  ...
  const unsigned aBegin = wA * %(BLOCK_SIZE)s * by;
  // Index of the last sub-matrix of A processed by the block
  const unsigned aEnd = aBegin + wA - 1;
  // Step size used to iterate through the sub-matrices of A
  const unsigned aStep = %(BLOCK_SIZE)s;

  // Index of the first sub-matrix of B processed by the block
  const unsigned bBegin = %(BLOCK_SIZE)s * bx;
  // Step size used to iterate through the sub-matrices of B
  const unsigned bStep = %(BLOCK_SIZE)s * wB;
  ...
  const unsigned c = wB * %(BLOCK_SIZE)s * by + %(BLOCK_SIZE)s * bx;
  C[c + wB * ty + tx] = Csub;
}
"""
